...
Note: Big Data/Data Lake: MinIO(s3-compatable object storage) with Hive(s3) SQL-Engine/DWH instead of Snowflake(SaaS/DWaaS) as big data solution on a single data
platform (DWH, S3, etc.), Apache Spark (Hive for metadata) + Delta Lake + Jupyter instead of Databricks (cloud-based big data processing SaaS). For production Analytics/ML platform we can also use cloud-native: Snowflake + Databricks + AWS S3/MKS/SQS/ELK/Lambda/etc.+ etc.
