...
Note: MinIO(s3) + Hive DWH(s3) instead of Snowflake (DWH, S3, etc.), Apache Spark (Hive for metadata) + Delta Lake + Jupyter instead of Databricks. For production we can use cloud-native: Snowflake + Databricks + AWS MKS/SQS/ELK/Lambda/etc.+ etc.
